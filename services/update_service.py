...
class ZnunyService:
    ...
    def get_ticket_attachments(self, ticket_id: int, session_id: str) -> List[Dict[str, Any]]:
        """
        Retrieves image attachments from the ticket articles.
        Returns a list of dicts: {'mime_type': str, 'data': bytes, 'filename': str}
        """
        headers = {"Accept": "application/json"}
        # Ensure we request all articles and their attachments
        url_ticket = f"{self.base_url}/Ticket/{ticket_id}?SessionID={session_id}&AllArticles=1"
        ...
        try:
            ...
            if not isinstance(articles, list):
                if articles: # if configured as dict for single article
                    articles = [articles]
                else:
                    articles = []

            for article in articles:
                article_attachments = article.get("Attachment", [])
                if not article_attachments:
                    continue
                    
                # Ensure list
                if not isinstance(article_attachments, list):
                        ...
                        if content_b64:
                            try:
                                ...
                                attachments.append({
                                    ...
                                    "data": decoded_data,
                                    "filename": att.get("Filename", "unknown.png")
                                })
                            except Exception as e:
                                logger.warning(f"Failed to decode attachment {att.get('Filename')}: {e}")
            
            return attachments

        except Exception as e:
            logger.error(f"Error fetching attachments for ticket {ticket_id}: {e}")
            return []
    ...
    def diagnose_and_update_ticket(self, ticket_id: int, session_id: Optional[str] = None, data: Optional[dict] = None) -> Dict[str, Any]:
        ...
        # Try to get ticket text from payload first (for testing/simulation)
        ticket_text = None
        if "Article" in data and isinstance(data["Article"], dict):
            body = data["Article"].get("Body", "").strip()
            if body:
                ticket_text = body
                logger.info(f"Using ticket text from payload ({len(body)} chars)")

        # Fallback: fetch from Znuny API
        if not ticket_text:
        ...
        # 3. Obtener imágenes adjuntas
        images = []
        try:
            images = self.get_ticket_attachments(ticket_id, session_id)
            if images:
                logger.info(f"📸 Found {len(images)} images in ticket. Attaching to AI prompt.")
        except Exception as e:
            logger.error(f"❌ Error getting attachments: {e}")

        # Pass tool_config instead of examples_context
        response_data = self.agent_service.diagnose_ticket(ticket_text, tool_config, images=images) 
        ...
        if not diagnosis_body:
            raise RuntimeError("AI returned empty diagnosis.")
            
        # Ensure it is a string for the body
        if not isinstance(diagnosis_body, str):
            diagnosis_body = json.dumps(diagnosis_body, indent=2, ensure_ascii=False)
            
        if not diagnosis_body.strip():
            raise RuntimeError("AI returned empty diagnosis string.")
